...
//
//*****************************************************************************
#define SYSTICK_RELOAD_VALUE    0x418937
...
SysTickHandler(void)
{
    ...
    unsigned long ClockRate = PRCMPeripheralClockGet(PRCM_TIMERA0);
    char ClockRateString[10];
    sprintf(ClockRateString, "%ld", ClockRate);
      
    Message(ClockRateString);
    Message("\n\r");
    ...
    // Alert to the user
    Message("Completed SysTick Interrupt Handler \n\r");
...
SysTickInit(void)
{
    ...
    SysTickPeriodSet(4200000/*SYSTICK_RELOAD_VALUE*/);
    SysTickEnable();
